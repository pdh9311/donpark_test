...
int	checkformat(va_list ap, char *s)
{
	...
	while (s[i])
	{
		if (s[i] == '%')
		{
			conversion = findsdx(s + i + 1);	///  % 다음 위치를 넘겨줌.
			// printf("con = %c\n", *conversion);
			if (conversion)
			{	// fstrndup s + i 하는 것이 포인트!
				format = ft_strndup(s + i, (conversion + 1) - (s + i)); /// %부터  서식지정자까지의 길이
				//printf("format = %s\n", format);
				ret += printwithflags(format, *conversion, ap);
				printf("(i: %ld)", (conversion) - (s + i));
				i += ((conversion) - (s + i));	/// % 다음부터 서식지정자까지의 길이(즉, 서식지정자 위치까지 이동)
			}
